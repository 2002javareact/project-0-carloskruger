...
export async function daoFindUserByUsernameAndPassword(
	...
	try {
		...
		let results = await client.query(
			'SELECT * FROM ers.users U inner join ers.roles R on U."roleId" = R."roleId"  WHERE username = $1  and "password" = $2',
			[username, password]
		);
...
export async function daoFindAllUsers(): Promise<User[]> {
	...
	try {
		client = await connectionPool.connect();
		let results = await client.query(
			'SELECT * FROM ers.users U inner join ers.roles R on U."roleId" = R."roleId"'
		);
...
export async function daoSaveOneUser(newUser: UserDTO): Promise<User> {
	...
	try {
		...
		let roleId = (
			await client.query("SELECT * FROM ers.roles WHERE role = $1", [
				newUser.role
			])
		).rows[0].roleId;
		// send an insert that uses the id above and the user input
		let result = await client.query(
			'INSERT INTO ers.users (username, "password", email, firstName, lastName, "role") values ($1,$2,$3,$4,$5,$6) RETURNING userId;',
			[
				newUser.username,
				newUser.password,
				newUser.email,
...
export async function daoFindUserById(id: number): Promise<User> {
	...
	try {
		...
		let result = await client.query(
			'SELECT * FROM ers.users U inner join ers.roles R on U."roleId" = R."roleId" WHERE U."userId" = $1',
			[id]
		);
...
export async function daoUpdateUser(
	updates: object[],
	id: number
): Promise<User> {
	let client: PoolClient;
	try {
		...
		for (let i = 0; i < updates.length; i++) {
			...
			if (updateUsername === true && newUsername) {
				await client.query(
					'update ers.users set username = $1 where users."userId" = $2',
					[newUsername, id]
				);
			}

			if (updatePassword === true && newPassword) {
				await client.query(
					'update ers.users set "password" = $1 where users."userId" = $2',
					[newPassword, id]
				);
			}

			if (updateFirstName === true && newFirstName) {
				await client.query(
					'update ers.users set "firstName" = $1 where users."userId" = $2',
					[newFirstName, id]
				);
			}

			if (updateLastName === true && newLastName) {
				await client.query(
					'update ers.users set "lastName"= $1 where users."userId" = $2',
					[newLastName, id]
				);
			}

			if (updateEmail === true && newEmail) {
				await client.query(
					'update ers.users set email = $1 where users."userId" = $2',
					[newEmail, id]
				);
			}

			if (updateRole === true && newRole) {
				await client.query(
					'update ers.users set role= $1 where users."userId" = $2',
					[newRole, id]
				);
			}
		}
